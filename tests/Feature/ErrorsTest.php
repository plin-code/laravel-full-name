...
it('throws on unsupported relation type via searchFullName macro', function (): void {
    expect(fn () => Booking::query()->searchFullName('mario', relation: 'items')->get())
        ->toThrow(UnsupportedRelationException::class);
});

it('throws on missing relation via searchFullName macro', function (): void {
    expect(fn () => Booking::query()->searchFullName('mario', relation: 'totallyMadeUp')->get())
        ->toThrow(UnsupportedRelationException::class);
});

it('throws on invalid direction via orderByFullName macro', function (): void {
    expect(fn () => Booking::query()->orderByFullName('sideways')->get())
        ->toThrow(InvalidSortDirectionException::class);
});

it('throws on unsupported relation type via orderByFullName macro', function (): void {
